...
ASCharacter::ASCharacter() {
  ...
  GetCapsuleComponent()->SetCollisionResponseToChannel(COLLISION_WEAPON,
                                                       ECR_Ignore);
  ...
  BeamEndPointDecal =
      CreateDefaultSubobject<UDecalComponent>(TEXT("BeamEndPointDecal"));
  ...
  LagCompensationComp =
      CreateDefaultSubobject<ULagCompensationComp>(TEXT("LagCompensationComp"));
...
void ASCharacter::BindASCInput() {
  if (!bASCInputBound && IsValid(AbilitySystemComponent) &&
      IsValid(InputComponent)) {
	const FTopLevelAssetPath AbilityEnumAssetPath = FTopLevelAssetPath(FName("/Script/CoopTPS"), FName("EGSAbilityInputID"));
...
// networking -->
void ASCharacter::GetLifetimeReplicatedProps(
...
// Esta funcion es llamada desde el Gamemode para se?alar el cambio de color
// segun el player controller.
void ASCharacter::AuthSetPlayerColor(const FLinearColor &NewColor) {
  PlayerColor = NewColor;
  OnRep_PlayerColor();
}

const FHitResult &ASCharacter::GetHitTrace() const { return TraceResult; }
...
ESpawnActorCollisionHandlingMethod::AlwaysSpawn; if(StarterWeaponClass)
    {
...
FVector::ZeroVector, FRotator::ZeroRotator, SpawnParams);
    ...
    {
            SecondaryWeapon =
GetWorld()->SpawnActor<ASWeapon>(SecondaryWeaponClass, FVector::ZeroVector,
FRotator::ZeroRotator, SpawnParams); if(IsValid(SecondaryWeapon))
            {
                    SecondaryWeapon->SetOwner(this);
                    SecondaryWeapon->EquipWeapon(GetMesh(),
...
void ASCharacter::CreateRewindHitBox() {
  ...
  HitBox_Spine_02 =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_Spine_02"));
  ...
  HitBox_Spine_03 =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_Spine_03"));
  ...
  HitBox_UpperArm_l =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_UpperArm_l"));
  ...
  HitBox_UpperArm_r =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_UpperArm_r"));
  ...
  HitBox_lowerarm_l =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_lowerarm_l"));
  ...
  HitBox_lowerarm_r =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_lowerarm_r"));
  ...
  HitBox_thigh_l =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_thigh_l"));
  ...
  HitBox_thigh_r =
      CreateDefaultSubobject<UBoxComponent>(TEXT("HitBox_thigh_r"));
...
void ASCharacter::Tick(float DeltaTime) {
  ...
  float const NewFOV = FMath::FInterpTo(CameraComp->FieldOfView, TargetFOV,
...
USHealthComponent *ASCharacter::I_GetHealthComp() const { return HealthComp; }

bool ASCharacter::IsWeaponEquipped() const { return CurrentWeapon != nullptr; }

const TMap<FName, UBoxComponent *> &ASCharacter::GetRewindHitBoxes() const {
  return ServerRewindHitBoxes;
}

bool ASCharacter::IsAiming() const { return bIsAiming; }
...
void ASCharacter::I_Reload() {
  if (IsValid(CurrentWeapon)) {
    if (CurrentWeapon->GetCurrentAmmo() == CurrentWeapon->GetWeaponMaxAmmo() ||
        CurrentWeapon->IsReloading() || !CurrentWeapon->HaveAmmoInMag()) {
...
void ASCharacter::Throw() {
  if (GranadaClass) {
    ...
    if (const auto &Grenade = GetWorld()->SpawnActorDeferred<ASProjectile>(
            GranadaClass, FTransform(SpawnRotation, StartLocation))) {
      Grenade->InitialLocalVelocity = InitialLocalVelocity;
      UGameplayStatics::FinishSpawningActor(
          Grenade, FTransform(SpawnRotation, StartLocation, SpawnScale));
    }
  }
}

void ASCharacter::I_StartThrow() { bIsGranadaMode = true; }
...
void ASCharacter::AddNewBeam(FVector const NewPoint1, FVector const NewPoint2) {
  BeamComp = UGameplayStatics::SpawnEmitterAtLocation(
      GetWorld(), BeamFX, NewPoint1, FRotator::ZeroRotator, true);
...
// ReSharper disable once CppMemberFunctionMayBeStatic
void ASCharacter::GetSegmentAtTime(const FVector LocalStartLocation,
                                   const FVector LocalInitialVelocity,
                                   const FVector LocalGravity,
                                   const float LocalTime1,
                                   const float LocalTime2, FVector &OutPoint1,
                                   FVector &OutPoint2) {
...
void ASCharacter::DrawingTrajectory() {
  if (GranadaClass) {
    ...
    FVector ThrowRotateVector =
        GetControlRotation().RotateVector(GrenadeOffset);
    StartLocation =
        GetMesh()->GetSocketLocation(GrenadeSocketName) + ThrowRotateVector;
    const FTransform TotalPosition(SpawnRotation, ThrowRotateVector,
                                   SpawnScale);

    FVector InitialVelocity = UKismetMathLibrary::TransformDirection(
        TotalPosition, InitialLocalVelocity);
...
FLinearColor ASCharacter::IterationTrace() {
  ...
  if (FHitResult HitResult; GetWorld()->LineTraceSingleByChannel(
    ...
    if (DebugCharacter) {
      DrawDebugSphere(GetWorld(), HitResult.ImpactPoint, 5.f, 12, FColor::White,
                      false, 1.f, 0, 3);
    }
...
void ASCharacter::PickupWeapon() {
  if (IsValid(OverlappingWeapon)) {
    ...
    if (IsLocallyControlled() && HasAuthority()) {
      PlayerController->SetWeaponInfoHUD(CurrentWeapon->GetWeaponName(),
...
void ASCharacter::TurnInPlace() {
  ...
  if (bTurnInPlace) {
    InterpAimYaw =
        FMath::FInterpTo(InterpAimYaw, 0.f, GetWorld()->GetDeltaSeconds(), 5.f);
...
void ASCharacter::CalculateAimOffset() {
  ...
  if (GetVelocity().Size() == 0.f && !GetCharacterMovement()->IsFalling()) {
    const FRotator CurrentAimRotation =
  ...
  if (AimOffSetPitch > 90.f && !IsLocallyControlled()) {
    ...
    AimOffSetPitch =
        FMath::GetMappedRangeValueClamped(InRange, OutRange, AimOffSetPitch);
  }
}

void ASCharacter::Multicast_PlayMontage_Implementation(
  ...
  if (IsLocallyControlled() && !HasAuthority()) {
    if (IsValid(CurrentWeapon)) {
      PlayerController->SetWeaponInfoHUD(CurrentWeapon->GetWeaponName(),
                                         CurrentWeapon->GetCurrentAmmo(),
                                         CurrentWeapon->GetAmmoInBackpack());
      UE_LOG(LogTemp, Warning, TEXT("Character : Current ammo on %s : %d"),
             *ShowPlayerNetRole(), CurrentWeapon->GetCurrentAmmo());
    } else {
      PlayerController->SetWeaponInfoHUD();
    }
  }
...
void ASCharacter::ServerSwapWeapon_Implementation() { SwapWeapons(); }
...
void ASCharacter::ServerPickupWeapon_Implementation() { PickupWeapon(); }

void ASCharacter::HealthChanged(USHealthComponent *OwningHealthComp,
                                const float Health, float HealthDelta,
...
void ASCharacter::SetHUDCrosshairs(float DeltaTime) {
  ...
  PlayerController = PlayerController == nullptr
                         ? Cast<ACoopPlayerController>(GetController())
                         : PlayerController;
  if (PlayerController) {
    ...
    if (HUD) {
      FHUDData HUDData;
      if (CurrentWeapon) {
        HUDData.CrosshairsCenter =
            CurrentWeapon->GetCrosshairData().CrosshairsCenter;
        HUDData.CrosshairsLeft =
            CurrentWeapon->GetCrosshairData().CrosshairsLeft;
        HUDData.CrosshairsRight =
            CurrentWeapon->GetCrosshairData().CrosshairsRight;
        HUDData.CrosshairsUp = CurrentWeapon->GetCrosshairData().CrosshairsUp;
        HUDData.CrosshairsDown =
            CurrentWeapon->GetCrosshairData().CrosshairsDown;
      } else {
      ...
      const FVector2d WalkSpeedRange(0.0f,
                                     GetCharacterMovement()->MaxWalkSpeed);
      const FVector2d VelocityMultiplierRange(0.0f, 1.f);
      FVector Velocity = GetVelocity();
      Velocity.Z = 0.f;
      CrosshairSpreadFactor = FMath::GetMappedRangeValueClamped(
          WalkSpeedRange, VelocityMultiplierRange, Velocity.Size());
      if (GetCharacterMovement()->IsFalling()) {
        CrossInAirFactor =
            FMath::FInterpTo(CrossInAirFactor, 2.25f, DeltaTime, 2.25f);
        ...
        CrossInAirFactor =
            FMath::FInterpTo(CrossInAirFactor, 0.f, DeltaTime, 30.f);
      }
